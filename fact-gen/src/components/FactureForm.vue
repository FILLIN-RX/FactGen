<template>
  <div class="flex flex-col lg:flex-row h-full">
    ...
    <div 
      ...
      :class="{ 'fixed inset-0 z-50 lg:relative lg:inset-auto lg:z-auto': showPreview }"
    >
      <div class="sticky top-0 bg-white border-b border-slate-200 p-4 lg:hidden">
        <button
          @click="togglePreview"
...
const factureInstance = computed(() => {
  const reductionData = utiliseReduction.value === 'oui' ? reduction.value : null
  return new Facture(societer.value, client.value, produits.value, reductionData)
})
...
const isFormValid = computed(() => {
  const validSocieter = societer.value.nom && societer.value.email && societer.value.adresse
  const validClient = client.value.nom
  const validProduits = produits.value.some(p => p.nom && p.prix > 0 && p.quantite > 0)
