<template>
    <div v-if="facture" class="bg-white p-6 mt-6 facture-container rounded-xl shadow-lg mx-auto font-sans">
        <div class="bg-white p-6 mt-6 rounded-xl shadow-lg mx-auto font-sans">
            ...
            <div class="mb-8" v-if="facture.produits && facture.produits.length > 0">
                ...
                <div v-for="(p, i) in facture.produits" :key="i"
...
onMounted(async () => {
    // Si une facture est passée en prop, on l'utilise directement
    if (props.invoice) {
        facture.value = props.invoice;
        loading.value = false;
        return;
    }

    // Sinon, on charge depuis l'API si on a un ID dans l'URL
    if (route.params.id) {
        try {
            loading.value = true;
