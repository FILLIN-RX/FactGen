<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30">
      ...
      <div class="px-3 py-2.5 sm:px-6 sm:py-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h1 class="text-base font-bold text-slate-900 sm:text-lg">Factures</h1>
              <p class="text-xs text-slate-500 leading-none">{{ facturesFiltrees.length }} résultat(s)</p>
            </div>
          </div>
    ...
    <div class="px-3 py-4 sm:px-6 sm:py-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div 
        class="mb-4 sm:mb-6 transition-all duration-300 overflow-hidden"
        :class="showFilters || !isMobile ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 lg:max-h-screen lg:opacity-100'"
      >
      ...
      <div v-if="open" class="mb-4 sm:mb-6">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden">
          <div class="px-3 py-2.5 sm:px-4 sm:py-3 lg:px-6 lg:py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <h3 class="text-base sm:text-lg font-semibold text-slate-900">Nouvelle facture</h3>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
        <div class="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-xl p-3 sm:p-4 lg:p-6 border border-blue-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs sm:text-sm font-medium text-blue-700">Total</p>
              <p class="text-lg sm:text-xl lg:text-2xl font-bold text-blue-900 mt-0.5">{{ invoiceStore.factures.length }}</p>
            </div>
            <div class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 bg-blue-600 rounded-lg flex items-center justify-center">
        ...
        <div class="bg-gradient-to-br from-emerald-50 to-emerald-100/50 rounded-xl p-3 sm:p-4 lg:p-6 border border-emerald-200/50">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs sm:text-sm font-medium text-emerald-700">Montant</p>
              <p class="text-sm sm:text-base lg:text-2xl font-bold text-emerald-900 mt-0.5">{{ formatPrice(totalAmount) }}€</p>
            </div>
            <div class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 bg-emerald-600 rounded-lg flex items-center justify-center">
              <svg class="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-amber-50 to-amber-100/50 rounded-xl p-3 sm:p-4 lg:p-6 border border-amber-200/50 col-span-2 lg:col-span-1">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs sm:text-sm font-medium text-amber-700">En attente</p>
              <p class="text-lg sm:text-xl lg:text-2xl font-bold text-amber-900 mt-0.5">{{ pendingCount }}</p>
            </div>
            <div class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 bg-amber-600 rounded-lg flex items-center justify-center">
              <svg class="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="!invoiceStore.hasInvoices" class="text-center py-12 px-4">
        ...
        <h3 class="text-lg sm:text-xl font-semibold text-slate-900 mb-2">Aucune facture</h3>
        <p class="text-sm sm:text-base text-slate-600 mb-6 max-w-sm mx-auto">Commencez par créer votre première facture pour gérer vos finances.</p>
        <button 
          @click="creer" 
      ...
      <div v-else-if="facturesFiltrees.length === 0" class="text-center py-12 px-4">
        ...
        <h3 class="text-lg sm:text-xl font-semibold text-slate-900 mb-2">Aucun résultat</h3>
        <p class="text-sm sm:text-base text-slate-600 mb-6">Modifiez vos critères de recherche.</p>
        <button 
          @click="clearFilters" 
      ...
      <div class="bg-white rounded-xl shadow-2xl max-w-sm w-full overflow-hidden">
        <div class="p-4 sm:p-6">
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 class="text-base sm:text-lg font-semibold text-slate-900">Confirmer la suppression</h3>
              <p class="text-xs sm:text-sm text-slate-600">Action irréversible</p>
            </div>
          </div>
