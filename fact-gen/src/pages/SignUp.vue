  ...
  <div class="relative min-h-screen flex items-center justify-center overflow-hidden">
    ...
    <div
      ...
      <div class="text-center mb-6">
        <router-link to="/" class="inline-flex items-center justify-center w-14 h-14 bg-[#1E40AF] rounded-xl shadow-lg mb-4 text-white text-2xl transition-all">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-5">
        ...
        <div class="relative">
          ...
          <span v-if="!errors.email && form.email" class="absolute right-3 top-3 text-green-500 material-symbols-outlined">check_circle</span>
        ...
        <div class="relative">
          ...
          <!-- Password Strength -->
          <div v-if="form.password" class="h-1.5 rounded-full mt-2" :class="getStrengthColor(passwordStrength)" :style="{ width: `${passwordStrength*25}%` }"></div>
          <p class="text-xs mt-1" :class="getStrengthTextColor(passwordStrength)">Force : {{ getStrengthText(passwordStrength) }}</p>
        ...
        <div class="relative">
          ...
          <span class="absolute left-3 top-3 text-gray-400 material-symbols-outlined">lock_reset</span>
          <button type="button" @click="showConfirmPassword = !showConfirmPassword"
            class="absolute right-3 top-3 text-gray-500">
            <span class="material-symbols-outlined">{{ showConfirmPassword ? 'visibility_off' : 'visibility' }}</span>
          </button>
          <span v-if="!errors.confirmPassword && form.confirmPassword && form.password===form.confirmPassword"
            class="absolute right-10 top-3 text-green-500 material-symbols-outlined">check_circle</span>
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm">J'accepte les <a href="#" class="text-[#1E40AF] underline">conditions</a></span>
        </div>
        ...
        <!-- Google Login -->
        <div class="mt-4">
          <button class="w-full flex items-center justify-center py-3 border rounded-lg bg-white hover:bg-gray-50 transition">
