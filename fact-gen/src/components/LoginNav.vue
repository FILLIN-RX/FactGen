...
const menuItems = [
  { id: 1, icon: 'menu', active: true },
  { id: 2, icon: 'dashboard', active: false },
  { id: 3, icon: 'people', active: false },
  { id: 4, icon: 'folder', active: false },
  { id: 5, icon: 'document', active: false },
  { id: 6, icon: 'chart', active: false },
  { id: 7, icon: 'mail', active: false },
  { id: 8, icon: 'settings', active: false },
  { id: 9, icon: 'logout', active: false }
];
</script>

<template>
  <aside class="w-16 bg-white shadow-md flex flex-col items-center py-6 h-screen">
    <div class="mb-8">
      <h1 class="font-bold text-xl text-primary-700">F</h1>
    </div>
      ...
      <ul class="space-y-6">
        <li v-for="item in menuItems" :key="item.id" 
            :class="['w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300', 
                    item.active ? 'bg-primary-500 text-white' : 'text-neutral-500 hover:bg-neutral-100']">
          ...
          <!-- Dashboard icon -->
          <svg v-else-if="item.icon === 'dashboard'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
          </svg>
          
          <!-- People icon -->
          <svg v-else-if="item.icon === 'people'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
